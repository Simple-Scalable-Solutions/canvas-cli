...
func newUsersMergeIntoServiceCredentialsForUsers2Cmd(flags *rootFlags) *cobra.Command {
	var stdinBody bool
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			path = replacePathParam(path, "destination_account_id", args[0])
			if len(args) < 2 {
				return usageErr(fmt.Errorf("destination_user_id is required\nUsage: %s <%s>", cmd.CommandPath(), "destination_user_id"))
			}
			path = replacePathParam(path, "destination_user_id", args[1])
			if len(args) < 3 {
