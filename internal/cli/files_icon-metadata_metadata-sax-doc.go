...
func newFilesIconMetadataMetadataSaxDocCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			path := "/files/{id}/icon_metadata"
			path = replacePathParam(path, "id", args[0])
			params := map[string]string{}
			data, prov, err := resolveRead(cmd.Context(), c, flags, "icon-metadata", false, path, params, nil)
			if err != nil {
