...
func newCoursesExternalFeedsDestroyForCoursesCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) < 2 {
				return usageErr(fmt.Errorf("external_feed_id is required\nUsage: %s <%s>", cmd.CommandPath(), "external_feed_id"))
			}
			path = replacePathParam(path, "external_feed_id", args[1])
