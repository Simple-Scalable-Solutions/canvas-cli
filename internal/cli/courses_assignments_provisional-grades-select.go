...
func newCoursesAssignmentsProvisionalGradesSelectCmd(flags *rootFlags) *cobra.Command {
	var stdinBody bool
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) < 3 {
				return usageErr(fmt.Errorf("provisional_grade_id is required\nUsage: %s <%s>", cmd.CommandPath(), "provisional_grade_id"))
			}
			path = replacePathParam(path, "provisional_grade_id", args[2])
