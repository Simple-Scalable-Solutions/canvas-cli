...
func newPermissionsHelpPermissionsCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			path := "/permissions/{context_type}/{permission}/help"
			path = replacePathParam(path, "context_type", args[0])
			if len(args) < 2 {
				return usageErr(fmt.Errorf("permission is required\nUsage: %s <%s>", cmd.CommandPath(), "permission"))
			}
			path = replacePathParam(path, "permission", args[1])
			params := map[string]string{}
			data, prov, err := resolveRead(cmd.Context(), c, flags, "help", false, path, params, nil)
			if err != nil {
