...
func newUsersTemporaryEnrollmentStatusEnrollmentsApiShowCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			path := "/users/{user_id}/temporary_enrollment_status"
			path = replacePathParam(path, "user_id", args[0])
			params := map[string]string{}
			if flagAccountId != "" {
				params["account_id"] = fmt.Sprintf("%v", flagAccountId)
			}
			data, prov, err := resolveRead(cmd.Context(), c, flags, "temporary-enrollment-status", false, path, params, nil)
			if err != nil {
	...
	cmd.Flags().StringVar(&flagAccountId, "account-id", "", "The ID of the account to check for temporary enrollment status. Defaults to the domain root account if not provided.")
