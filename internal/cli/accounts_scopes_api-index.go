...
func newAccountsScopesApiIndexCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cmd.Flags().Changed("group-by") {
				allowedGroupBy := []string{ "resource_name" }
				validGroupBy := false
			...
			if flagGroupBy != "" {
				params["group_by"] = fmt.Sprintf("%v", flagGroupBy)
			}
			data, prov, err := resolveRead(cmd.Context(), c, flags, "scopes", false, path, params, nil)
			if err != nil {
	...
	cmd.Flags().StringVar(&flagGroupBy, "group-by", "", "The attribute to group the scopes by. By default no grouping is done. (one of: resource_name)")
