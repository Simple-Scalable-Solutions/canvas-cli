...
func newCoursesTabsUpdateCmd(flags *rootFlags) *cobra.Command {
	var bodyHidden bool
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) < 2 {
				return usageErr(fmt.Errorf("tab_id is required\nUsage: %s <%s>", cmd.CommandPath(), "tab_id"))
			}
			path = replacePathParam(path, "tab_id", args[1])
	...
	cmd.Flags().IntVar(&bodyPosition, "position", 0, "The new position of the tab, 1-based")
