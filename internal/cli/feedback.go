...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CANVAS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CANVAS_FEEDBACK_AUTO_SEND")))
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.canvas-pp-cli/feedback.jsonl.
When ` + "`CANVAS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CANVAS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
